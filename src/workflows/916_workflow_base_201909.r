...
envg$EXPENV$exp_dir <- "~/buckets/b1/expw_predic/"
envg$EXPENV$wf_dir <- "~/buckets/b1/flow_predic/"
...
TS_strategy_base6 <- function( pinputexps )
{
  ...
  param_local$future <- c(201909)

  param_local$final_train$undersampling <- 0.2
  param_local$final_train$clase_minoritaria <- c( "BAJA+1", "BAJA+2")
  param_local$final_train$training <- c( 201908, 201907, 201906 )
  
  param_local$train$training <- c(201906,201905,201904)
  param_local$train$validation <- c(201907)
  param_local$train$testing <- c(201908)
...
wf_Exp_stacking_w201909 <- function( pnombrewf )
{
  param_local <- exp_wf_init( pnombrewf ) # linea workflow inicial fija

  # Etapa especificacion dataset de la Segunda Competencia Kaggle
  DT_incorporar_dataset( "~/buckets/b1/datasets/competencia_03_ct.csv.gz")
  ...
  ht <- HT_tuning_base( bo_iteraciones = 5 )  # iteraciones inteligentes

  # Etapas finales
  fm <- FM_final_models_lightgbm( c(ht, ts6), ranks=c(1), qsemillas=1 )
  SC_scoring( c(fm, ts6) )
